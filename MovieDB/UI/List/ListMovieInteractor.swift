...
class ListMovieInteractor: ListMoviePresenterToInteractor {
    ...
    func getGenres() {
        Task { [weak self] in
            ...
            do {
                ...
                presenter?.didLoadGenres(genres: genres)
            } catch {
    ...
    private func fetchMovies(isNewGenre: Bool) {
        ...
        fetchTask = Task { [weak self] in
            guard let self else { return }
            do {
                let movies = try await repository.getMovies(page: page, genreId: selectedGenreId)
                guard counter == trackRequestedGenreId else { return }
                page += 1
                isFetching = false
                presenter?.didLoadMovies(movies: movies, isNewGenre: isNewGenre)
            } catch {
                guard counter == trackRequestedGenreId else { return }
                isFetching = false
                presenter?.didFailed(message: error.localizedDescription)
